...
export class CountryComponent implements OnInit {
  constructor(
    ...
    this.columnDefs = [
      ...
      { field: 'code', headerName: `${this.i18n.fanyi('function.country.grid.code')}`, minWidth: 180, flex: 1 },
      {
        field: 'name',
        headerName: `${this.i18n.fanyi('function.country.grid.name')}`,
        sortable: true,
        filter: true,
        minWidth: 180,
        flex: 1,
      ...
      {
        field: 'status',
        headerName: `${this.i18n.fanyi('function.country.grid.status')}`,
        minWidth: 150,
  ...
  @ViewChild(CountryItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  initRightOfUser(): void {
    this.btnAdd.grandAccess = this.aclService.canAbility('COUNTRY-ADD');
    this.btnDelete.grandAccess = this.aclService.canAbility('COUNTRY-DELETE');
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.countryApiService.delete(listId).subscribe(
      (res: any) => {
  ...
  async getById(id: any): Promise<any> {
    return this.countryApiService.getById(id).toPromise();
  }

  initGridData(): Subscription {
    this.btnDelete.visible = false;
    this.gridApi.showLoadingOverlay();
    const rs = this.countryApiService.getFilter(this.filter).subscribe(
      (res: any) => {
        ...
        for (const item of dataResult.data) {
          ...
          item.editGrantAccess = this.aclService.canAbility('COUNTRY-EDIT');
          item.deleteGrantAccess = this.aclService.canAbility('COUNTRY-DELETE');
        }
