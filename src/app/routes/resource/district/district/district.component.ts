...
export class DistrictComponent implements OnInit {
  constructor(
    ...
    this.columnDefs = [
      ...
      { field: 'code', headerName: `${this.i18n.fanyi('function.district.grid.code')}`, minWidth: 180, flex: 1 },
      { field: 'name', headerName: `${this.i18n.fanyi('function.district.grid.name')}`, sortable: true, filter: true, minWidth: 180, flex: 1 },
      { field: 'provinceName', headerName: `${this.i18n.fanyi('function.district.grid.provincename')}`, sortable: true, filter: true, minWidth: 180, flex: 1 },
      { field: 'status', headerName: `${this.i18n.fanyi('function.district.grid.status')}`, minWidth: 150, cellRenderer: 'statusNameCellRender' },
      {
  ...
  @ViewChild(DistrictItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string, option: any) => void };
  ...
  tittle = `${this.i18n.fanyi('function.district.title')}`;
  ...
  ngOnInit(): void {
    ...
    this.loadComboBoxData(); // gọi list danh sách tỉnh thành
  }

  initRightOfUser(): void {
    this.btnAdd.grandAccess = this.aclService.canAbility('DISTRICT-ADD');
    this.btnDelete.grandAccess = this.aclService.canAbility('DISTRICT-DELETE');
  ...
  onAddItem(): void {
    ...
    this.itemModal.initData({}, 'add', {listProvinceName: this.listProvinceName});
  }
...
async  onEditItem(item: any = null): Promise<void> {
    ...
    this.itemModal.initData(res.data, 'edit', {listProvinceName: this.listProvinceName});
  }
  ...
  async onViewItem(item: any = null): Promise<void>{
    ...
    this.itemModal.initData(res.data, 'info', {listProvinceName: this.listProvinceName});
  }
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.districtApiService.delete(listId).subscribe(
      (res: any) => {
 ...
 async getById(id: any): Promise<any>{
    return this.districtApiService.getById(id).toPromise();
  }

  initGridData(): Subscription {
    this.btnDelete.visible = false;
    this.gridApi.showLoadingOverlay();
    const rs = this.districtApiService.getFilter(this.filter).subscribe(
      (res: any) => {
        ...
        for (const item of dataResult.data) {
          ...
          item.editGrantAccess = this.aclService.canAbility('DISTRICT-EDIT');
          item.deleteGrantAccess = this.aclService.canAbility('DISTRICT-DELETE');
        }
