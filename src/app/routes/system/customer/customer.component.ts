...
import { DocumentApiService, OrganizationApiService, UserService } from '@service';
import {
...
import { DocumentCellRenderComponent } from 'src/app/shared/ag-grid/cell-render/document-cell-render/document-cell-render.component';
import { UserV2ItemComponent } from '../../user/user-item/user-item-v2.component';
...
export class CustomerComponent implements OnInit {
  constructor(
    ...
    this.columnDefs = [
      ...
      {
        field: 'eformStatus',
        headerName: `${this.i18n.fanyi('function.user.grid.status-eform')}`,
        minWidth: 250,
        cellRenderer: 'eformStatusCellRender',
        cellRendererParams: {
          btnDaKyCTS: (item: any) => this.btnDaKyCTS(item),
          btnDaKyGDDTAT: (item: any) => this.btnDaKyGDDTAT(item),
      ...
      {
        field: 'documentCode',
        headerName: `${this.i18n.fanyi('function.user.grid.document-code')}`,
        minWidth: 250,
        cellRenderer: 'documentCellRender',
        cellRendererParams: {
          btnDocumentInfoClick: (item: any) => this.btnDocumentInfoClick(item),
          btnDocumentDropdownClick: (item: any) => this.btnDocumentDropdownClick(item),
        },
        flex: 1,
      },
      {
        ...
        cellRendererParams: {
          ...
          // addRoleClicked: (item: any) => this.addRoleItem(item),
    ...
    this.frameworkComponents = {
      ...
      documentCellRender: DocumentCellRenderComponent,
    };
  ...
  @ViewChild(UserV2ItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string, isCustomer?: boolean) => void };
  ...
  listRole: any[] = [];
  listDocumentByUser: any[] = [];
  listUserId: any[] = [];

  btnDocumentInfoClick(data: any) {
    const documentIds = [data.documents.documents[0].documentId];
    this.documentItemModal.initData(documentIds, 'info');
  }

  btnDocumentDropdownClick(data: any) {
  ...
  async getListDocumentByListUser() {
    const res = await this.documentService.getListDocumentByListUserId(this.listUserId).toPromise();
    this.listDocumentByUser = res.data;
  }
  ...
  onEditItem(item: any = null): void {
    ...
    this.itemModal.initData(item, 'edit', true);
  }
  ...
  onViewItem(item: any = null): void {
    ...
    this.itemModal.initData(item, 'info', true);
  }
  ...
  onLockItem(item: any = null): void {
    ...
    this.changeStateModal.initData(selectedRows, content);
  }
  ...
  initGridData(): Subscription {
    ...
    this.filter.isInternalUser = false;
    const rs = this.userService.getFilter(this.filter).subscribe(
      async (res: any) => {
        ...
        const dataResult = res.data;
        if (dataResult.data) {
          this.listUserId = dataResult.data.map((x: any) => x.id);
        }

        await this.getListDocumentByListUser();
        ...
        for (const item of dataResult.data) {
          ...
          item.editGrantAccess = this.aclService.canAbility('CUSTOMER-EDIT');
          ...
          // item.addRoleGrantAccess = true;
          // item.signConfigGrantAccess = true;

          // 22-9-2021
          // item.hsmAccountGrantAccess = true;

          item.eformStatus = null;
          if (item.userEFormInfo) {
            const lstStatus = [];
            if (item.userEFormInfo.isConfirmDigitalSignature) {
              lstStatus.push(`${this.i18n.fanyi('function.customer.eform-confirm-digital-signature.label')}`);
            }
            if (item.userEFormInfo.isConfirmRequestCertificate) {
              lstStatus.push(`${this.i18n.fanyi('function.customer.eform-confirm-request-certificate.label')}`);
            }
            ...
            item.eformStatus = `${this.i18n.fanyi('function.customer.eform-not-found.label')}`;
          }

          item.documents = this.listDocumentByUser.find((x) => x.userId === item.id);
        }
  ...
  btnDaKyCTS(item: any) {
    if (item.userEFormInfo) {
      const documentIds = [];
      if (item.userEFormInfo.requestCertificateDocumentId && item.userEFormInfo.isConfirmRequestCertificate) {
