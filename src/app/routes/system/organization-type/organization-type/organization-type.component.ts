...
import { OrganizationTypeImportItemComponent } from '../organization-type-import-item/organization-type-import-item.component';
import { OrganizationTypeItemComponent } from '../organization-type-item/organization-type-item.component';
...
@Component({
  ...
  styleUrls: ['./organization-type.component.less'],
})
export class OrganizationTypeComponent implements OnInit {
  constructor(
    ...
    this.columnDefs = [
      ...
      { field: 'code', headerName: `${this.i18n.fanyi('function.organization-type.grid.code')}`, minWidth: 180, flex: 1 },
      {
        field: 'name',
        headerName: `${this.i18n.fanyi('function.organization-type.grid.name')}`,
        sortable: true,
        filter: true,
        minWidth: 180,
        flex: 1,
      ...
      {
        field: 'createdDate',
        headerName: `${this.i18n.fanyi('function.organization-type.grid.createddate')}`,
        minWidth: 150,
        cellRenderer: 'dateCellRender',
      },
      {
        field: 'status',
        headerName: `${this.i18n.fanyi('function.organization-type.grid.status')}`,
        minWidth: 150,
  ...
  @ViewChild(OrganizationTypeItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  @ViewChild(OrganizationTypeImportItemComponent, { static: false }) importModal!: { initData: () => void };
  ...
  tittle = `${this.i18n.fanyi('function.organization-type.title')}`;
  ...
  initRightOfUser(): void {
    this.btnAdd.grandAccess = this.aclService.canAbility('ORGANIZATION-TYPE-ADD');
    this.btnDelete.grandAccess = this.aclService.canAbility('ORGANIZATION-TYPE-DELETE');
  }
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.organizationTypeApiService.delete(listId).subscribe(
      (res: any) => {
  ...
  initGridData(): Subscription {
    ...
    const rs = this.organizationTypeApiService.getFilter(this.filter).subscribe(
      (res: any) => {
        ...
        for (const item of dataResult.data) {
          ...
          item.editGrantAccess = this.aclService.canAbility('ORGANIZATION-TYPE-EDIT');
          item.deleteGrantAccess = this.aclService.canAbility('ORGANIZATION-TYPE-DELETE');
        }
