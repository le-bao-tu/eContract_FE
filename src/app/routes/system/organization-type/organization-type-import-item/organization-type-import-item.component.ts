...
//mport { CertifyTypeApiService } from './../../../../services/api/certify-type-api.service';
...
import { DateCellRenderComponent, StatusImportCellRenderComponent, StatusNameCellRenderComponent } from '@shared';

@Component({
  selector: 'app-organization-type-import-item',
  templateUrl: './organization-type-import-item.component.html',
  styleUrls: ['./organization-type-import-item.component.less'],
})
export class OrganizationTypeImportItemComponent implements OnInit {
  constructor(
  ...
  tittle = 'Nhập dữ liệu loại hợp đồng từ excel';
  ...
  save(): Subscription {
    ...
    const promise = this.organizationTypeApiService.createMany(data).subscribe(
      (res: any) => {
        this.isLoading = false;
        if (res.code !== 200) {
