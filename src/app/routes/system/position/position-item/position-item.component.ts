...
import { PositionApiService } from '@service';
...
@Component({
  ...
  styleUrls: ['./position-item.component.less'],
})
export class PositionItemComponent implements OnInit {
  ...
  public initData(data: any, type: any = null, option: any = {}): void {
    ...
    if (this.item.id === null || this.item.id === undefined) {
      this.form = this.fb.group({
        code: [{ value: null, disabled: this.isInfo }],
      ...
      this.form = this.fb.group({
        code: [{ value: this.item.code, disabled: this.isInfo }],
  ...
  save(isCreateAfter: boolean = false): Subscription | undefined {
    ...
    if (data.code === null || data.code === undefined || data.code === '') {
      this.isLoading = false;
      this.messageService.error(`Mã chức vụ không được để trống!`);
      return;
    }

    if (data.name === null || data.name === undefined || data.name === '') {
      this.isLoading = false;
      this.messageService.error(`Tên chức vụ không được để trống!`);
      return;
    }

    if (this.isAdd) {
      const promise = this.positionApiService.create(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
      ...
      const promise = this.positionApiService.update(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
            this.messageService.error(`${res.message}`);
