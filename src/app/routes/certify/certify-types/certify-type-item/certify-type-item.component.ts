...
@Component({
  selector: 'app-certify-type-item',
  templateUrl: './certify-type-item.component.html',
  styleUrls: ['./certify-type-item.component.less'],
})
export class CertifyTypeItemComponent implements OnInit {
  @Input() type = 'add';
  @Input() item: any;
  ...
  initRightOfUser(): void {
    this.btnEdit.grandAccess = this.aclService.canAbility('DOCUMENT-TYPE-EDIT');
    this.btnSaveAndCreate.grandAccess = this.aclService.canAbility('DOCUMENT-TYPE-EDIT') || this.aclService.canAbility('DOCUMENT-TYPE-ADD');
  }
  ...
  updateFormType(type: string): void {
    switch (type) {
      case 'add':
        ...
        this.tittle = `${this.i18n.fanyi('function.certify-type.modal-item.header-add')}`;
        break;
      case 'info':
        this.isInfo = true;
        this.isEdit = false;
        ...
        this.tittle = `${this.i18n.fanyi('function.certify-type.modal-item.header-info')}`;
        break;
      case 'edit':
        this.isInfo = false;
        this.isEdit = true;
        ...
        this.tittle = `${this.i18n.fanyi('function.certify-type.modal-item.header-edit')}`;
        break;
      default:
        this.isInfo = false;
        this.isEdit = false;
  ...
  save(isCreateAfter: boolean = false): Subscription | undefined {
    ...
    if (!this.form.valid) {
      this.isLoading = false;
      this.messageService.error(`${this.i18n.fanyi('function.certify-type.modal-item.error.message.form-invalid')}`);
      return;
    }
    ...
    if (this.isAdd) {
      const promise = this.certifyTypeApiService.create(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
      ...
      const promise = this.certifyTypeApiService.update(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
            this.messageService.error(`${res.message}`);
