...
import { InfoInCertifyImportItemComponent } from './../info-in-certify-import-item/info-in-certify-import-item.component';
import { InfoInCertifyItemComponent } from './../info-in-certify-item/info-in-certify-item.component';
...
@Component({
  selector: 'app-info-in-certify',
  templateUrl: './info-in-certify.component.html',
  styleUrls: ['./info-in-certify.component.less'],
})
export class InfoInCertifyComponent implements OnInit {
  constructor(
    ...
    this.columnDefs = [
      ...
      { field: 'code', headerName: `${this.i18n.fanyi('function.info-in-certify.grid.code')}`, minWidth: 180, flex: 1 },
      {
        field: 'name',
        headerName: `${this.i18n.fanyi('function.info-in-certify.grid.name')}`,
        sortable: true,
        filter: true,
        minWidth: 180,
        flex: 1,
      },
      // { field: 'description', headerName: 'Ghi chú', sortable: true, filter: true, minWidth: 180, flex: 1 },
      {
        field: 'createdDate',
        headerName: `${this.i18n.fanyi('function.info-in-certify.grid.createddate')}`,
        minWidth: 150,
        cellRenderer: 'dateCellRender',
      },
      {
        field: 'status',
        headerName: `${this.i18n.fanyi('function.info-in-certify.grid.status')}`,
        minWidth: 150,
    ...
    this.btnImportExcel = {
      title: 'Import excel',
      titlei18n: `${this.i18n.fanyi('layout.button.btn-importexcel.label')}`,
      visible: true,
      enable: true,
      grandAccess: true,
      click: ($event: any) => {
  ...
  @ViewChild(InfoInCertifyItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  @ViewChild(InfoInCertifyImportItemComponent, { static: false }) importModal!: { initData: () => void };
  ...
  tittle = `${this.i18n.fanyi('function.info-in-certify.title')}`;
  moduleName = 'danh sách thông tin trong hợp đồng';
  ...
  initRightOfUser(): void {
    this.btnAdd.grandAccess = this.aclService.canAbility('META-DATA-ADD');
    this.btnDelete.grandAccess = this.aclService.canAbility('META-DATA-DELETE');
    this.btnImportExcel.grandAccess = this.aclService.canAbility('META-DATA-IMPORT');
    this.btnExportExcel.grandAccess = this.aclService.canAbility('META-DATA-EXPORT');
  }
        ...
        [
          {
            ...
            data: {
              type: 'String',
              value: `Danh sách ${this.moduleName}`,
            },
            mergeAcross: 4,
          },
        ],
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.infoInCertifyApiService.delete(listId).subscribe(
      (res: any) => {
  ...
  initGridData(): Subscription {
    ...
    const rs = this.infoInCertifyApiService.getFilter(this.filter).subscribe(
      (res: any) => {
        ...
        for (const item of dataResult.data) {
          ...
          item.editGrantAccess = this.aclService.canAbility('META-DATA-EDIT');
          item.deleteGrantAccess = this.aclService.canAbility('META-DATA-DELETE');
        }
