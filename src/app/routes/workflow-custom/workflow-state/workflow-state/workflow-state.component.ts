...
import { WorkflowStateItemComponent } from '../workflow-state-item/workflow-state-item.component';

@Component({
  selector: 'app-workflow-state',
  templateUrl: './workflow-state.component.html',
  styleUrls: ['./workflow-state.component.less'],
})
export class WorkflowStateComponent implements OnInit {
  constructor(
    ...
    this.columnDefs = [
      ...
      { field: 'code', headerName: `${this.i18n.fanyi('function.workflow-state.grid.code')}`, minWidth: 180, flex: 1 },
      {
        field: 'name',
        headerName: `${this.i18n.fanyi('function.workflow-state.grid.name')}`,
        sortable: true,
        filter: true,
        minWidth: 180,
        flex: 1,
      ...
      {
        field: 'nameForReject',
        headerName: `${this.i18n.fanyi('function.workflow-state.grid.name-reject')}`,
        sortable: true,
        filter: true,
        minWidth: 180,
        flex: 1,
      ...
      {
        field: 'createdDate',
        headerName: `${this.i18n.fanyi('function.workflow-state.grid.createddate')}`,
        minWidth: 150,
        cellRenderer: 'dateCellRender',
      },
      {
        field: 'status',
        headerName: `${this.i18n.fanyi('function.workflow-state.grid.status')}`,
        minWidth: 150,
  ...
  @ViewChild(WorkflowStateItemComponent, { static: false }) itemModal!: { initData: (arg0: {}, arg1: string) => void };
  ...
  tittle = `${this.i18n.fanyi('function.workflow-state.title')}`;
  ...
  initRightOfUser(): void {
    this.btnAdd.grandAccess = this.aclService.canAbility('WORKFLOW-STATE-ADD');
    this.btnDelete.grandAccess = this.aclService.canAbility('WORKFLOW-STATE-DELETE');
  }
  ...
  deleteListItem(listId: [string]): Subscription {
    ...
    const promise = this.workflowStateApiService.delete(listId).subscribe(
      (res: any) => {
  ...
  initGridData(): Subscription {
    ...
    const rs = this.workflowStateApiService.getFilter(this.filter).subscribe(
      (res: any) => {
        ...
        for (const item of dataResult.data) {
          item.index = ++i;
          item.statusName = this.listStatus.find((x) => x.id === true)?.name;
          item.infoGrantAccess = true;
          item.editGrantAccess = this.aclService.canAbility('WORKFLOW-STATE-EDIT');
          item.deleteGrantAccess = this.aclService.canAbility('WORKFLOW-STATE-DELETE');
        }
