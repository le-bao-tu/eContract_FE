...
@Component({
  selector: 'app-workflow-state-item',
  templateUrl: './workflow-state-item.component.html',
  styleUrls: ['./workflow-state-item.component.less'],
})
export class WorkflowStateItemComponent implements OnInit {
  @Input() type = 'add';
  @Input() item: any;
  ...
  form: FormGroup;
  moduleName = 'trạng thái bước quy trình';
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      nameForReject: [null, [Validators.required]],
      order: [null],
      status: [true],
      description: [null],
    });
  ...
  initRightOfUser(): void {
    this.btnEdit.grandAccess = this.aclService.canAbility('WORKFLOW-STATE-EDIT');
    this.btnSave.grandAccess = this.aclService.canAbility('WORKFLOW-STATE-EDIT') || this.aclService.canAbility('WORKFLOW-STATE-ADD');
  }

  updateFormToEdit(): void {
    this.updateFormType('edit');
    this.form.get('name')?.enable();
    this.form.get('nameForReject')?.enable();
  ...
  updateFormType(type: string): void {
    switch (type) {
      case 'add':
        ...
        this.tittle = `${this.i18n.fanyi('function.workflow-state.modal-item.header-add')}`;
        break;
      case 'info':
        this.isInfo = true;
        this.isEdit = false;
        ...
        this.tittle = `${this.i18n.fanyi('function.workflow-state.modal-item.header-info')}`;
        break;
      case 'edit':
        this.isInfo = false;
        this.isEdit = true;
        ...
        this.tittle = `${this.i18n.fanyi('function.workflow-state.modal-item.header-edit')}`;
        break;
      default:
        this.isInfo = false;
        this.isEdit = false;
  ...
  public initData(data: any, type: any = null, option: any = {}): void {
    ...
    if (this.item.id === null || this.item.id === undefined) {
      this.form = this.fb.group({
        ...
        nameForReject: [{ value: null, disabled: this.isInfo }, [Validators.required, Validators.pattern(this.regexName)]],
      ...
      this.form = this.fb.group({
        ...
        nameForReject: [
          { value: this.item.nameForReject, disabled: this.isInfo },
          [Validators.required, Validators.pattern(this.regexName)],
        ],
  ...
  save(isCreateAfter: boolean = false): Subscription | undefined {
    ...
    if (!this.form.valid) {
      this.isLoading = false;
      this.messageService.error(`${this.i18n.fanyi('function.workflow-state.modal-item.error.message.form-invalid')}`);
      return;
    }
    ...
    const data = {
      ...
      nameForReject: this.form.controls.nameForReject.value,
    ...
    if (data.name === null || data.name === undefined || data.name === '') {
      this.isLoading = false;
      this.messageService.error(`Tên trạng thái hợp đồng không được để trống!`);
      return;
    }
    if (data.nameForReject === null || data.nameForReject === undefined || data.nameForReject === '') {
      this.isLoading = false;
      this.messageService.error(`Tên trạng thái hủy hợp đồng không được để trống!`);
      return;
    }

    if (this.isAdd) {
      const promise = this.workflowStateApiService.create(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
      ...
      const promise = this.workflowStateApiService.update(data).subscribe(
        (res: any) => {
          this.isLoading = false;
          if (res.code !== 200) {
            this.messageService.error(`${res.message}`);
