...
import { WorkflowApiService } from '@service';
import { StatusImportCellRenderComponent, StatusNameCellRenderComponent } from '@shared';

@Component({
  selector: 'app-workflow-import-item',
  templateUrl: './workflow-import-item.component.html',
  styleUrls: ['./workflow-import-item.component.less'],
})
export class WorkflowImportItemComponent implements OnInit {
  constructor(private messageService: NzMessageService, private workflowService: WorkflowApiService, private elementRef: ElementRef) {
    //#region Button
    this.btnDownload = {
      title: 'Tải về mẫu',
      titlei18n: '',
  ...
  save(): Subscription {
    ...
    const promise = this.workflowService.createMany(data).subscribe(
      (res: any) => {
        this.isLoading = false;
        if (res.code !== 200) {
